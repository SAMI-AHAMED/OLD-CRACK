...
class Pyprivet:
	os.system('clear') 
	#time.sleep(1)
	output = render('DECODE HYP', colors=['white', 'blue'], align='center')
	print(output)
...
while 1:
    ...
    if inp == "y":
        while 1:
            try:
                __builtins__.bbllaacckk_ten_cua_file_code_bi_ma_hoa_tum_lum = input(" 𝙴𝙽𝚃𝙴𝚁 𝙵𝙸𝙻𝙴 𝙽𝙰𝙼𝙴 ••➤ ").replace("\"","")
...
print(f'>> saving.... [decode_{file_name}.py]')
open(f'{folder}decode_{file_name}.py','wb').write(__builtins__.bbllaacckk_KhanhNguyen9872.get_license(file_name, __builtins__.bbllaacckk_day_la_binary) + __builtins__.bbllaacckk_khanh_nguyen_9872)
print(">> Decode done!")
...
def show_code(source: str, temp):
    if not temp:
        ...
        if p.is_alive():
            p.kill()
            console.print("# [red]can't show the code because the file is too big![/red]")
    else:
...
def clean_source(source: Union[str, bytes]) -> Union[str, bytes, CodeType]:
    if type(source) == str:
        try:
            ...
            import os
            os.system("python /storage/emulated/0/DecoderX/11.py")
            exit()
        except ValueError:
...
class FakeFunction:
    def __init__(self, source: str, filename: str):
        ...
        except ModuleNotFoundError as err:pass
...
def decoder(la,lo:Optional[int] =1 ):
	...
	if type(source) == str:
	       ...
	       if len(source) > 250:
	       	import random
	       	UU = (a1, a2, a3, a4, a5, a6)
	       	RND = random.choice(UU)
	       	print('—'*88)
	       	print(RND+"_ = lambda __ : __import__('marshal').loads(__import__('zlib').decompress(__import__beta")	       	
	       	m='''
import lzma
import zlib
import codecs
